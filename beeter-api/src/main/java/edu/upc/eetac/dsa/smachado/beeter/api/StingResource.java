...
public class StingResource {
	//Para CRUD, necesitamos una referencia al DataSource
	...
	// Obtener colección de stings por subject y content
	@GET
	@Path("/search")
	@Produces(MediaType.BEETER_API_STING_COLLECTION)
	public StingCollection getStings(@QueryParam("length") int length,
	...
	public Sting createSting(Sting sting) {
		...
		validateSting(sting); // Comprobamos que es un Sting válido 
		...
		PreparedStatement stmt = null;
		try {
			// Este return pide que le devuelvan la Primary
			// Key autogenerada (en la BD es un autoincrement)
			stmt = conn.prepareStatement(INSERT_STING_QUERY,
					Statement.RETURN_GENERATED_KEYS);
			
			// Damos valor a los datos stmt.setString(1, sting.getUsername());
