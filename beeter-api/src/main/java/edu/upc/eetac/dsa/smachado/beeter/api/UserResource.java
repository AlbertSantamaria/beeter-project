...
public class UserResource {
	...
	private void validateUser(User user) {
		...
		// Validar parametros validos en los campos de usuario
	...
	private void validateUpdateUser(User user) {
		...
		if (user.getName() != null && user.getName().length() > 70)
			throw new BadRequestException(
					"Name can't be longer than 70 characters.");
	...
	public User createUser(User user) {
		...
		try {
			...
			// Desactiva commit automático para asegurar que se añade a las 2
			...
			// Al hacer commit inserta en la DB
			conn.commit();
			...
			try {
				...
				if (stmtInsertUserIntoUserRoles != null)
					stmtGetUsername.close();
				// Para poder reutilizar la cx, reactivamos el autocommit porque
				// sino no se llegará a cerrar y no se podrá usar el pool
	...
	public User login(User user) {
		...
		// se hace el hash del password y se compara con el almacenado en la bd
	...
	private User getUserFromDatabase(String username, boolean password) {
		...
		// El boolean password define si se quiere recuperar o no el pwd de la
		// BD
	...
	// Método cambiar datos usuario (solo puede cambiar mail y nombre)
	@PUT
	@Path("/{username}")
	@Consumes(MediaType.BEETER_API_USER)
	@Produces(MediaType.BEETER_API_USER)
